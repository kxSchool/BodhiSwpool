...
class agent extends base
{
    ...
    function setCluster($redis)
    {
        ...
        //连接数据库
        $db = new mysqli($master->hostname, $master->username, $master->password, $master->database, $master->port) or die($master->hostname . '打开失败'); //连接数据库
        //设置查询结果编码
    ...
    {
        //连接数据库
        $db = new mysqli($rows->hostname, $rows->username, $rows->password, $rows->database, $rows->port) or die($rows->hostname . '打开失败');
        //设置查询结果编码
        $db->set_charset($rows->charset);
    ...
    function checkCluster($redis)
    {
        ...
        $slaves = $cluster->db_slaves;
        $db = new mysqli($master->hostname, $master->username, $master->password, $master->database, $master->port) or die($master->hostname . '打开失败'); //连接数据库
        $db->set_charset($master->charset); //设置查询结果编码
	...
	function status($redis){
        ...
        if(!empty($cluster->local)) {
            ...
            $db = new mysqli($local->hostname, $local->username, $local->password, $local->database, $local->port) or die($local->hostname . '打开失败'); //连接数据库
            $db->set_charset($local->charset); //设置查询结果编码
            $sql = "show master status";
            $result = $db->query($sql);
            $row_slave = $result->fetch_assoc();
